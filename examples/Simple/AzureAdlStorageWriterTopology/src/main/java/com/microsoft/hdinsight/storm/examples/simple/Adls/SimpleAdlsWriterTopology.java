package com.microsoft.hdinsight.storm.examples.simple.Adls;
...
import com.microsoft.hdinsight.storm.hdfs.bolt.AzureAdlsBolt;
import com.microsoft.hdinsight.storm.hdfs.bolt.format.HourlyFileNameFormat;

public class SimpleAdlsWriterTopology {

	private static final Logger LOG = LoggerFactory.getLogger(SimpleAdlsWriterTopology.class);

	public static void main(String[] args) throws Exception {
		Options options = new Options();

		Option topologyNameOption = new Option("n", "topologyName", true,
				"Topology Name Example: adlsPerfTestTopology");
		topologyNameOption.setRequired(true);
		options.addOption(topologyNameOption);

		Option ADLSUrlOption = new Option("u", "ADLSRepoUrl", true,
				"ADLS File repo url. Example: ADLS:/// (This uses the default ADLS storage account)");
		ADLSUrlOption.setRequired(true);
		options.addOption(ADLSUrlOption);

		Option ADLSFileRootOption = new Option("fr", "ADLSFileRoot", true,
				"Root folder for files. Example: \"/simpleadltest/\"");
		ADLSFileRootOption.setRequired(true);
		options.addOption(ADLSFileRootOption);
		...
		Option fileRotationSizeOption = new Option("rs", "fileRotationSize", true,
				"Rotation policy size for ADLS files (MB). 1024MB is a recommended size.");
		...
		try {
			...
			ADLSUrl = cmd.getOptionValue("ADLSRepoUrl");
			ADLSFileDirPath = cmd.getOptionValue("ADLSFileRoot");
		...
		System.out.println("ADLS File Rotation size (MB): " + fileRotationSize);
		System.out.println("ADLS File Repo: " + ADLSUrl);
		System.out.println("ADLS File container: " + ADLSFileDirPath);
		...
		FileNameFormat fileNameFormat2 = new HourlyFileNameFormat().withPath(ADLSFileDirPath);
		AbstractAzureHdfsBolt adlsBolt = new AzureAdlsBolt()
				.withRecordFormat(new DelimitedRecordFormat().withFieldDelimiter(",")).withFsUrl(ADLSUrl)
				.withRotationPolicy(new FileSizeRotationPolicy(fileRotationSize, Units.MB))
				.withFileNameFormat(fileNameFormat2);

		builder.setBolt("hdfsBolt", adlsBolt, boltParallelism).localOrShuffleGrouping("textgenerator");
